...
import { EmbassyLetterSubmission, SubmissionStatus, getStatusLabel, getStatusColor } from '../../types/submissions';

interface EmbassyLetterSubmissionDetailProps {
  submissionId: string;
  onBack: () => void;
  language: 'en' | 'my';
}

export function EmbassyLetterSubmissionDetail({ submissionId, onBack, language }: EmbassyLetterSubmissionDetailProps) {
  const [paymentProof, setPaymentProof] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Get submission from localStorage
  const getSubmission = (): EmbassyLetterSubmission | null => {
    const stored = localStorage.getItem('embassyLetterSubmissions');
    if (!stored) return null;
    const submissions: EmbassyLetterSubmission[] = JSON.parse(stored);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="p-4 space-y-6 pb-20">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{language === 'en' ? 'Request Information' : 'တောင်းဆိုမှုအချက်အလက်များ'}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div>
              <p className="text-sm text-muted-foreground mb-1">{language === 'en' ? 'Purpose of Letter:' : 'စာ၏ရည်ရွယ်ချက်:'}</p>
              <p className="font-medium">{submission.purposeOfLetter}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground mb-1">{language === 'en' ? 'Request Details:' : 'တောင်းဆိုချက်အသေးစိတ်:'}</p>
              <p className="font-medium">{submission.requestDetails}</p>
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="border-green-300 bg-green-50">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-sm text-green-800">
                {language === 'en' 
                  ? 'Your Embassy Letter request has been processed successfully.' 
                  : 'သင်၏ သံရုံးစာတောင်းဆိုမှုကို အောင်မြင်စွာ စီမံဆောင်ရွက်ပြီးပါပြီ။'}
              </p>
            </CardContent>
          </Card>
        )}
