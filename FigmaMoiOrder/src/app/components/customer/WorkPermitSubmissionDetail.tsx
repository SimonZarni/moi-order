...
import { WorkPermitSubmission, SubmissionStatus, getStatusLabel, getStatusColor } from '../../types/submissions';

interface WorkPermitSubmissionDetailProps {
  submissionId: string;
  onBack: () => void;
  language: 'en' | 'my';
}

export function WorkPermitSubmissionDetail({ submissionId, onBack, language }: WorkPermitSubmissionDetailProps) {
  const [paymentProof, setPaymentProof] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Get submission from localStorage
  const getSubmission = (): WorkPermitSubmission | null => {
    const stored = localStorage.getItem('workPermitSubmissions');
    if (!stored) return null;
    const submissions: WorkPermitSubmission[] = JSON.parse(stored);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="p-4 space-y-6 pb-20">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="grid grid-cols-2 gap-2 text-sm">
              <p className="text-muted-foreground">{language === 'en' ? 'Full Name:' : 'အမည်:'}</p>
              ...
              <p className="text-muted-foreground">{language === 'en' ? 'Nationality:' : 'နိုင်ငံသား:'}</p>
              <p className="font-medium">{submission.nationality}</p>
              
              <p className="text-muted-foreground">{language === 'en' ? 'NRC:' : 'မှတ်ပုံတင်:'}</p>
              <p className="font-medium">{submission.nrcNumber}</p>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{language === 'en' ? 'Employment Information' : 'အလုပ်အကိုင်အချက်အလက်များ'}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid grid-cols-2 gap-2 text-sm">
              <p className="text-muted-foreground">{language === 'en' ? 'Employer:' : 'အလုပ်ရှင်:'}</p>
              <p className="font-medium">{submission.employerName}</p>
              
              <p className="text-muted-foreground">{language === 'en' ? 'Position:' : 'ရာထူး:'}</p>
              <p className="font-medium">{submission.jobPosition}</p>
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="border-green-300 bg-green-50">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-sm text-green-800">
                {language === 'en' 
                  ? 'Your Work Permit application has been processed successfully.' 
                  : 'သင်၏ အလုပ်ခွင့်လိုင်စင်လျှောက်ထားမှုကို အောင်မြင်စွာ စီမံဆောင်ရွက်ပြီးပါပြီ။'}
              </p>
            </CardContent>
          </Card>
        )}
