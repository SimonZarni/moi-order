...
9. **Mapbox `PointAnnotation` — NO live animation possible.** The component snapshots the React Native view as a bitmap at mount time. Neither Reanimated worklets (native thread) nor RN `Animated` with `useNativeDriver: false` (JS thread) propagate updates after the initial snapshot. The **only** way to change the visual state of a marker is to change the `key` prop, which forces a full remount with the correct static styles baked in. Always use static conditional styles (e.g. `isSelected && styles.selected`) and include every state variable that affects appearance in the key (e.g. `key={\`marker-${id}-${isSelected ? 's' : 'n'}\`}`). Smooth animation inside `PointAnnotation` is not achievable with the current Mapbox RN SDK.
