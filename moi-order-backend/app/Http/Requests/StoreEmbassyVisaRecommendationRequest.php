...
/**
 * Principle: SRP — owns all validation + authorisation for an embassy visa recommendation submission.
 * Principle: Security — MIME validated in rules(); service ownership validated in withValidator().
 *   All four documents are identity documents — images only.
 */
class StoreEmbassyVisaRecommendationRequest extends FormRequest
{
    ...
    public function withValidator(\Illuminate\Validation\Validator $validator): void
    {
        $validator->after(function (\Illuminate\Validation\Validator $v): void {
            ...
            if ($type->service->slug !== 'embassy-visa-recommendation') {
                $v->errors()->add(
                    'service_type_id',
                    'The selected service type does not belong to the Embassy Visa Recommendation service.'
                );
            }
        });
    }
