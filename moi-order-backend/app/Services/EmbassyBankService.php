...
use App\Models\EmbassyBankDetail;
...
/**
 * Principle: SRP — owns all business logic for embassy bank submissions only.
...
class EmbassyBankService
{
    ...
    /**
     * Create a new embassy bank submission.
     * Idempotent: returns the existing submission if the key was already used by this user.
     */
    public function create(CreateEmbassyBankDTO $dto): ServiceSubmission
    {
        $existing = ServiceSubmission::with(['serviceType.service', 'embassyBankDetail', 'documents'])
            ->forUser($dto->userId)
        ...
        return DB::transaction(function () use ($dto, $serviceType): ServiceSubmission {
            ...
            EmbassyBankDetail::create([
                'submission_id'   => $submission->id,
                'full_name'       => $dto->fullName,
                'passport_no'     => $dto->passportNo,
                'identity_card_no'=> $dto->identityCardNo,
                'current_job'     => $dto->currentJob,
                'company'         => $dto->company,
                'myanmar_address' => $dto->myanmarAddress,
                'thai_address'    => $dto->thaiAddress,
                'phone'           => $dto->phone,
                'bank_name'       => $dto->bankName,
            ]);

            $this->storeDocument($submission->id, DocumentType::PassportSizePhoto,  $dto->passportSizePhoto,  self::IMAGE_MIMES);
            ...
            return $submission->load(['serviceType.service', 'embassyBankDetail', 'documents']);
        });
    }
