...
use App\Models\EmbassyResidentialDetail;
...
/**
 * Principle: SRP — owns all business logic for embassy residential submissions only.
 ...
 */
class EmbassyResidentialService
{
    ...
    /**
     * Create a new embassy residential submission.
     * Idempotent: returns the existing submission if the key was already used by this user.
     */
    public function create(CreateEmbassyResidentialDTO $dto): ServiceSubmission
    {
        $existing = ServiceSubmission::with(['serviceType.service', 'embassyResidentialDetail', 'documents'])
            ->forUser($dto->userId)
        ...
        return DB::transaction(function () use ($dto, $serviceType): ServiceSubmission {
            ...
            EmbassyResidentialDetail::create([
            ...
            return $submission->load(['serviceType.service', 'embassyResidentialDetail', 'documents']);
        });
    }
