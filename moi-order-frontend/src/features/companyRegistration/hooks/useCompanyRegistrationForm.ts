...
export interface CompanyRegistrationFormState {
  fullName:          string;
  phone:             string;
...
export interface UseCompanyRegistrationFormResult {
  form: CompanyRegistrationFormState;
  ...
  handleTm30Change:              (asset: ImagePickerAsset) => void;
  validate:      () => boolean;
  applyApiError: (errors: Record<string, string[]>) => void;
}

const FIELD_MAP: Record<string, keyof CompanyRegistrationFormState> = {
  full_name:           'fullName',
  phone:               'phone',
...
export function useCompanyRegistrationForm(): UseCompanyRegistrationFormResult {
  const [form, setForm] = useState<CompanyRegistrationFormState>({
    fullName:          '',
    phone:             '',
  ...
  const handleTm30Change = useCallback((asset: ImagePickerAsset): void => {
    clearError('tm30');
    setForm((prev) => ({ ...prev, tm30: asset }));
  }, [clearError]);
  ...
  const validate = useCallback((): boolean => {
    ...
    if (form.tm30 === null)                 errors['tm30']              = 'TM30 is required.';
