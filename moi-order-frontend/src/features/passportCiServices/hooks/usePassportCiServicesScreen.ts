...
import { usePassportCiServices, UsePassportCiServicesResult } from './usePassportCiServices';
...
export interface UsePassportCiServicesScreenResult {
  services: UsePassportCiServicesResult['services'];
  isLoading: boolean;
...
export function usePassportCiServicesScreen(): UsePassportCiServicesScreenResult {
  const navigation = useNavigation<Nav>();
  const { locale } = useLocale();
  const { services, isLoading, isRefreshing, isError, refetch } = usePassportCiServices();
