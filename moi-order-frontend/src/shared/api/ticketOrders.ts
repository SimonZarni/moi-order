...
export async function createTicketOrder(payload: CreateTicketOrderPayload): Promise<TicketOrder> {
  const { data } = await apiClient.post<ApiResponse<TicketOrder>>('/ticket-orders', payload);
  return data.data;
}

export async function fetchTicketOrders(page = 1): Promise<PaginatedResponse<TicketOrder>> {
  const { data } = await apiClient.get('/ticket-orders', { params: { page } });
  return data;
}

export async function fetchTicketOrder(id: number): Promise<TicketOrder> {
  const { data } = await apiClient.get<ApiResponse<TicketOrder>>(`/ticket-orders/${id}`);
  return data.data;
}

export async function createTicketOrderPayment(ticketOrderId: number): Promise<Payment> {
  const { data } = await apiClient.post<ApiResponse<Payment>>(`/ticket-orders/${ticketOrderId}/payment`);
  return data.data;
}

export async function fetchTicketOrderPayment(ticketOrderId: number): Promise<Payment> {
  const { data } = await apiClient.get<ApiResponse<Payment>>(`/ticket-orders/${ticketOrderId}/payment`);
  return data.data;
}

export async function syncTicketOrderPaymentStatus(ticketOrderId: number): Promise<void> {
  await apiClient.post(`/ticket-orders/${ticketOrderId}/payment/sync`);
}

export async function fetchTicketOrderEticketUrl(ticketOrderId: number): Promise<string> {
  const { data } = await apiClient.get<ApiResponse<{ url: string }>>(`/ticket-orders/${ticketOrderId}/eticket`);
